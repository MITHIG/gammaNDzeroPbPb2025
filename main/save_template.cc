...
int macro(std::string inputstr, std::string cutstr, std::string output) {
  std::cout<<std::endl;
  ...
  auto project = [&trs, &h3](std::string key, std::string icut) {
    h3[key] = new TH3F(Form("h3_%s", key.c_str()), ";y;m_{K#pi} [GeV];p_{T} [GeV]",
  ...
  project("match", cut+" && Dgen==23333");
  project("swap", cut+" && Dgen==23344");
