...
IFS=';' evtgaps=($evtgap)
cutdtopo=${5:-"((Dy<-1 && Dmva_BDT>0.143) || (Dy>=-1 && Dy<0 && Dmva_BDT>0.142) || (Dy>=0 && Dy<1 && Dmva_BDT>0.123) || (Dy>=1 && Dmva_BDT>0.098));;"}
...
[[ ${execu:0:1} -eq 1 ]] && {
    ...
    [[ $runlevel -eq 1 || $runlevel -eq 3 ]] && ./evteff_save.exe "$input_mc_evt" "$cutevtstr" "${cutdstr} && $CUT_SIGNALWIN" $itag_evteff "$BINNING_Y" "$BINNING_PT" #
    itag_evteff_calc=${itag_evteff/evteffsave/evteffcalc}
    [[ $runlevel -eq 2 || $runlevel -eq 3 ]] && ./evteff_calc.exe "rootfiles/"$itag_evteff".root" $itag_evteff_calc $SAVE_PNG #
}
...
[[ ${execu:1:1} -eq 1 ]] && {
    ...
    [[ $runlevel -eq 1 || $runlevel -eq 3 ]] && ./hist_save.exe "$input_template" "$cutstr" $itag_template "$BINNING_Y" "$BINNING_PT" 1 # 1: is_template
...
[[ ${execu:2:1} -eq 1 ]] && {
    ...
    [[ $runlevel -eq 1 || $runlevel -eq 3 ]] && ./hist_save.exe "$input_data" "$cutstr" $itag_data "$BINNING_Y" "$BINNING_PT" 0 # 0: not template
...
[[ ${execu:3:1} -eq 1 ]] && {
    ...
    [[ $runlevel -eq 2 || $runlevel -eq 3 ]] && ./hist_fit.exe "rootfiles/"$itag_data".root" "rootfiles/"$itag_template".root" $itag_data_fit "$fitopt" $SAVE_PNG
...
# itag_deff=$cut_tag"/deffsave_"$mc_tag
itag_deff_calc=$cut_tag_DEFAULT"/"$TAG_BINNING"/deffcalc_"$mc_tag
[[ ${execu:4:1} -eq 1 ]] && {
    itag_deff=$cut_tag"/deffsave_"$mc_tag # no binning info
    [[ ($runlevel -eq 1 || $runlevel -eq 3) && $TAG_BINNING == "b-default" ]] && ./eff_save.exe "$input_mc" "$cutevtstr" "$cutdstr" $itag_deff null
    itag_deff_calc=$cut_tag"/"$TAG_BINNING"/deffcalc_"$mc_tag
    [[ $runlevel -eq 2 || $runlevel -eq 3 ]] && ./eff_calc.exe "rootfiles/"$itag_deff".root" $itag_deff_calc "$BINNING_Y" "$BINNING_PT" $SAVE_PNG
