...
cutdtopos=(
    ...
    "`make_dcut_string 0.003,0.002,-0.017,-0.042`;BDT shift -0.12;-bdtsM0p14"
    ...
    "`make_dcut_string 0.243,0.242,0.223,0.198`;BDT shift 0.08;-bdts0p10"
)
    ...
    for evtgap in "${evtgaps[@]}" ; do
        ...
        for cutdtopo in "${cutdtopos[@]}" ; do
            ...
            for fitopt in "${fitopts[@]}" ; do
                ...
                [[ $def_evtzdc -eq 0 ]] && ./run_var.sh $isNgamma 00110 "$evtzdc" "$evtgap" "$cutdtopo" "$fitopt" ${1:-0}
                [[ $def_evtgap -eq 0 ]] && ./run_var.sh $isNgamma 10110 "$evtzdc" "$evtgap" "$cutdtopo" "$fitopt" ${1:-0} &
                [[ $def_cutdtopo -eq 0 ]] && ./run_var.sh $isNgamma 01111 "$evtzdc" "$evtgap" "$cutdtopo" "$fitopt" ${1:-0} &
                [[ $def_fitopt -eq 0 ]] && ./run_var.sh $isNgamma 00010 "$evtzdc" "$evtgap" "$cutdtopo" "$fitopt" ${1:-0}
