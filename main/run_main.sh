...
INPUTS_DATA=(
    "/eos/cms/store/group/phys_heavyions/wangj/Forest2025PbPb/Dzero_260426-yrefmva_PbPbUPC_HIForward_Dpt-2_Dsize_24PD.root;2025 PbPb (5.36 TeV);2025PbPb;"
...
CUT_BASE="Dpt>=2 && Dpt<5 && fabs(Dy)<2 && TMath::Abs(Dtrk1PtErr/Dtrk1Pt)<0.1 && TMath::Abs(Dtrk2PtErr/Dtrk2Pt)<0.1 && Dtrk1Pt > 0.5 && Dtrk2Pt > 0.5 && TMath::Abs(Dtrk1Eta) < 2.4 && TMath::Abs(Dtrk2Eta) < 2.4 && Dchi2cl > 0.05 && (DsvpvDistance/DsvpvDisErr) > 1. && DsvpvDisErr>1.e-8 && DsvpvDisErr_2D>1.e-8"
CUTDS=(
...
make save_hist.exe fit_hist.exe eff_save.exe eff_calc.exe || exit 1
...
for cutevtstr in "${CUTEVTS[@]}" ; do
    ...
    for cutdstr in "${CUTDS[@]}" ; do
        ...
        for input_data in "${INPUTS_DATA[@]}" ; do
            ...
            echo -e "  \033[33m"$data_tag"\033[0m: "$data_lumi" mb-1"
