...
make save_datasets.exe fit_datasets.exe || exit 1
...
for cutevtstr in "${CUTEVTS[@]}" ; do
    ...
    for input_data in "${INPUTS_DATA[@]}" ; do
        ...
        itag_dataset_data="rootfiles/"$cut_tag"/"$data_tag
        [[ ${1:-0} -eq 1 ]] && {
        ...
        for input_mc in "${INPUTS_MC[@]}" ; do
            ...
            itag_dataset_mc="rootfiles/"$cut_tag"/"$mc_tag
            [[ ${2:-0} -eq 1 ]] && {
                ./save_datasets.exe "$input_mc" $itag_dataset_mc $cutevt 1
            }            

            itag_fit=$cut_tag"/"$data_tag"_"$mc_tag"_roofit"
            [[ ${3:-0} -eq 1 ]] && {
                ./fit_datasets.exe $itag_dataset_data".root" $itag_dataset_mc".root" $itag_fit
            }
